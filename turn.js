    ...
    i = {
      ...
      _cornerActivated: function (a) {
        ...
        switch (data.effect) {
          ...
          // SHEET page (normal flipping pages)
          case "sheet":

            // FULL LEFT EDGE (top & bottom)
            if (a.x <= edgeSize) {
              a.corner = (a.y < height / 2) ? "tl" : "bl";
            }
            // FULL RIGHT EDGE (top & bottom)
            else if (a.x >= width - edgeSize) {
              a.corner = (a.y < height / 2) ? "tr" : "br";
            }
            else {
              return false;
            }
        ...
        // Respect allowed directions (LTR / RTL / first / last page)
        return (!a.corner || f.inArray(a.corner, allowed) === -1) ? false : a;
      },
      _isIArea: function (a) {
